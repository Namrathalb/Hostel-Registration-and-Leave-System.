...
public class LeaveForm extends JFrame {
	...
	public LeaveForm() {
		...
		JLabel lblAddBooks = new JLabel("LEAVE FORM");
		lblAddBooks.setForeground(Color.GRAY);
		lblAddBooks.setFont(new Font("Tahoma", Font.PLAIN, 18));
		
		JLabel lblCallNo = new JLabel("USN:");
		
		JLabel lblName = new JLabel("Room NO:");
		
		JLabel lblAuthor = new JLabel("Location");
		
		JLabel lblPublisher = new JLabel("Till Date");
		
		JLabel lblQuantity = new JLabel("Father no");
		...
		JButton btnAddBooks = new JButton("Leave Form");
		btnAddBooks.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			String usn=textField.getText();
			String roomno=textField_1.getText();
			String location=textField_2.getText();
			String going=textField_3.getText();
			String quantity=textField_4.getText();
			int squantity=Integer.parseInt(quantity);
			int i=LeaveDao.save(usn, roomno, location, going, squantity);
			if(i>0){
				JOptionPane.showMessageDialog(LeaveForm.this,"Leave form Submitted successfully!");
				StudentSucess.main(new String[]{});
				frame.dispose();
				
			}else{
				JOptionPane.showMessageDialog(LeaveForm.this,"Sorry, unable to save!");
			}
			}
